...
#!!!DEFINE DATASET/PATH: 
dataset <- read_csv("P:\\MicroData\\SILC\\Microdata_2024_October\\result\\SILC_C_2020_all.csv")
...
dataset <- dataset %>%
  mutate(
    # Step 1: Recode to ISCED levels
    ISCED = case_when(
      PE040 == 0 ~ 0,                                   # Pre-primary / less than primary
      PE040 == 100 ~ 1,                                 # Primary
      PE040 == 200 ~ 2,                                 # Lower secondary
      PE040 %in% c(300, 340:344, 350:354) ~ 3,          # Upper secondary
      PE040 %in% c(400, 440, 450) ~ 4,                  # Post-secondary non-tertiary
      PE040 == 500 ~ 5,                                 # Short-cycle tertiary
      PE040 >= 600 & PE040 < 700 ~ 6,                # Bachelor’s or equivalent
      PE040 >= 700 & PE040 < 800 ~ 7,                # Master’s or equivalent
      PE040 >= 800 ~ 8,                                 # Doctoral or higher
      TRUE ~ NA_real_
    ),
    
    # Step 2: Categorise into education levels
    educ = case_when(
      ISCED %in% 0:2 ~ "lower_secondary/ Low Education",
      ISCED %in% 3:4 ~ "upper_secondary / Medium Education",
      ISCED %in% 5:8 ~ "tertiary/ High Education",
      TRUE ~ NA_character_
    )
  )
...
Persons_withoutInternet_TotalPopulation_filtered_2020 <- Persons_WithoutInternet_TotalPopulation %>% 
  filter(PD080 == "No internet access")

Persons_withoutInternet_TotalPopulation_filtered_2020$time <- 2020

Persons_withoutInternet_TotalPopulation_filtered_2020 <- Persons_withoutInternet_TotalPopulation_filtered_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
...
Persons_withoutInternet_AROPE_filtered_1_2020 <- Persons_withoutInternet_AROPE %>%
  filter(PD080== "No internet access" & AROPE == "Yes, AROPE")

Persons_withoutInternet_AROPE_filtered_1_2020$time <- 2020

Persons_withoutInternet_AROPE_filtered_1_2020 <- Persons_withoutInternet_AROPE_filtered_1_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_notAROPE_filtered_2_2020 <- Persons_withoutInternet_AROPE %>%
  filter(PD080== "No internet access" & AROPE != "Yes, AROPE")

Persons_withoutInternet_notAROPE_filtered_2_2020$time <- 2020

Persons_withoutInternet_notAROPE_filtered_2_2020 <- Persons_withoutInternet_notAROPE_filtered_2_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AROP_filtered_1_2020 <- Persons_withoutInternet_AROP %>%
  filter(PD080 == "No internet access" & AROP == "Yes, AROP")

Persons_withoutInternet_AROP_filtered_1_2020$time <- 2020

Persons_withoutInternet_AROP_filtered_1_2020 <- Persons_withoutInternet_AROP_filtered_1_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_notAROP_filtered_2_2020 <- Persons_withoutInternet_AROP %>%
  filter(PD080 == "No internet access" & AROP != "Yes, AROP")

Persons_withoutInternet_notAROP_filtered_2_2020$time <- 2020

Persons_withoutInternet_notAROP_filtered_2_2020 <- Persons_withoutInternet_notAROP_filtered_2_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AgeGroups_Ages1617_2020 <- Persons_withoutInternet_AgeGroups %>%
  filter(PD080 == "No internet access" & age_group == "16-17 years old")

Persons_withoutInternet_AgeGroups_Ages1617_2020$time <- 2020

Persons_withoutInternet_AgeGroups_Ages1617_2020 <- Persons_withoutInternet_AgeGroups_Ages1617_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AgeGroups_Ages1864_2020 <- Persons_withoutInternet_AgeGroups %>%
  filter(PD080 == "No internet access" & age_group == "18-64 years old")

Persons_withoutInternet_AgeGroups_Ages1864_2020$time <- 2020

Persons_withoutInternet_AgeGroups_Ages1864_2020 <- Persons_withoutInternet_AgeGroups_Ages1864_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AgeGroups_Ages65Over_2020 <- Persons_withoutInternet_AgeGroups %>%
  filter(PD080 == "No internet access" & age_group == "65+ years")

Persons_withoutInternet_AgeGroups_Ages65Over_2020$time <- 2020

Persons_withoutInternet_AgeGroups_Ages65Over_2020 <- Persons_withoutInternet_AgeGroups_Ages65Over_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_GENDER_filtered_Men_2020 <- Persons_withoutInternet_GENDER %>%
  filter(PD080 == "No internet access" & PB150 == "Male")


Persons_withoutInternet_GENDER_filtered_Men_2020$time <- 2020

Persons_withoutInternet_GENDER_filtered_Men_2020 <- Persons_withoutInternet_GENDER_filtered_Men_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_GENDER_filtered_Female_2020 <- Persons_withoutInternet_GENDER %>%
  filter(PD080 == "No internet access" & PB150 == "Female")

Persons_withoutInternet_GENDER_filtered_Female_2020$time <- 2020

Persons_withoutInternet_GENDER_filtered_Female_2020 <- Persons_withoutInternet_GENDER_filtered_Female_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_URBANISATION_Urban_2020 <- Persons_withoutInternet_URBANISATION %>%
  filter(PD080 == "No internet access" & Urbanisation == "Urban Areas")

Persons_withoutInternet_URBANISATION_Urban_2020$time <- 2020

Persons_withoutInternet_URBANISATION_Urban_2020 <- Persons_withoutInternet_URBANISATION_Urban_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_URBANISATION_Rural_2020 <- Persons_withoutInternet_URBANISATION %>%
  filter(PD080 == "No internet access" & Urbanisation == "Rural Areas")

Persons_withoutInternet_URBANISATION_Rural_2020$time <- 2020

Persons_withoutInternet_URBANISATION_Rural_2020 <- Persons_withoutInternet_URBANISATION_Rural_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_Education_Low_2020 <- Persons_withoutInternet_Education %>%
  filter(PD080 == "No internet access" & educ == "lower_secondary/ Low Education")


Persons_withoutInternet_Education_Low_2020$time <- 2020

Persons_withoutInternet_Education_Low_2020 <- Persons_withoutInternet_Education_Low_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_Education_Medium_2020 <- Persons_withoutInternet_Education %>%
  filter(PD080 == "No internet access" & educ == "upper_secondary / Medium Education")

Persons_withoutInternet_Education_Medium_2020$time <- 2020

Persons_withoutInternet_Education_Medium_2020 <- Persons_withoutInternet_Education_Medium_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_Education_High_2020 <- Persons_withoutInternet_Education %>%
  filter(PD080 == "No internet access" & educ == "tertiary/ High Education")

Persons_withoutInternet_Education_High_2020$time <- 2020

Persons_withoutInternet_Education_High_2020 <- Persons_withoutInternet_Education_High_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_WithoutComputer_TotalPopulation_filtered_2020 <- Persons_WithoutComputer_TotalPopulation %>%
  filter(HS090 == "No computer")


Persons_WithoutComputer_TotalPopulation_filtered_2020$time <- 2020

Persons_WithoutComputer_TotalPopulation_filtered_2020 <- Persons_WithoutComputer_TotalPopulation_filtered_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_AROP_filtered_1_2020 <-  Persons_withoutComputer_AROP %>%
  filter(HS090 == "No computer" & AROP == "Yes, AROP")


Persons_withoutComputer_AROP_filtered_1_2020$time <- 2020

Persons_withoutComputer_AROP_filtered_1_2020 <- Persons_withoutComputer_AROP_filtered_1_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_notAROP_filtered_2_2020 <-  Persons_withoutComputer_AROP %>%
  filter(HS090 == "No computer" & AROP == "No, not AROP")

Persons_withoutComputer_notAROP_filtered_2_2020$time <- 2020

Persons_withoutComputer_notAROP_filtered_2_2020 <- Persons_withoutComputer_notAROP_filtered_2_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_URBANISATION_Urban_2020 <- Persons_withoutComputer_URBANISATION %>%
  filter(HS090 == "No computer" & Urbanisation == "Urban Areas")

Persons_withoutComputer_URBANISATION_Urban_2020$time <- 2020

Persons_withoutComputer_URBANISATION_Urban_2020 <- Persons_withoutComputer_URBANISATION_Urban_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_URBANISATION_Rural_2020 <- Persons_withoutComputer_URBANISATION %>%
  filter(HS090 == "No computer" & Urbanisation == "Rural Areas")

Persons_withoutComputer_URBANISATION_Rural_2020$time <- 2020

Persons_withoutComputer_URBANISATION_Rural_2020 <- Persons_withoutComputer_URBANISATION_Rural_2020 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
rm(list = setdiff(ls(), ls(pattern = "_2020")))

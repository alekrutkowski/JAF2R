...
Number_of_redefined_commented_out_indics_in_Codelines <- 0 # PA6a.S5.; PA9.1.S5.; PA9.2.S4.
...
processCatalog <- function(catalog_dt, comment="")
  ...
  .[, table := sub('tepsr_sp110 / and (gov_10a_exp)','gov_10a_exp',table,fixed=TRUE)] %>% 
  ...
  .[, func :=
      SOURCE %>% 
      {kit::nif(grepl('AMECO',.),'fromAMECO',
                grepl('^lfse_',table) & !grepl('^edat_lfse_',table), 'fromLFSspecialFile',
                ...
                (grepl("^Eurostat,",.) & table!='vacancy_rate') |
                  ...
                  table %in% c('earn_nt_unemtrp','earn_nt_taxwedge','ilc_mdsd07',
                               ...
                               'demo_mlexpecedu','isoc_sks_itspt','isoc_sks_itsps',
                               'isoc_ske_itts','gov_10a_exp','educ_uoe_enra21',
                               'educ_uoe_grad04','edat_lfse_03') |
                  grepl('tps00...',table),
                'fromEurostatDataset',
  ...
  .[, definitions := 
      definitions %>% 
      ifelse(grepl('fromEurostatDataset.*AMECO',.),
             gsub('fromEurostatDataset','fromAMECO',.,fixed=TRUE),
             .)] %>% 
  .[, definitions := 
      gsub('sex="W"','sex="F"',definitions,fixed=TRUE) %>% 
      gsub('sex="w"','sex="F"',.,fixed=TRUE) %>% 
      gsub('unit="PC_EMPL"','unit="PC_EMP"',.,fixed=TRUE) %>% 
      gsub('size_empl="','size_emp="',.,fixed=TRUE)] %>% 
  .[, JAF_KEY := JAF_KEY %>% 
      trimws() %>% gsub("\\s+", ".", .)] %>% 
  .[, definitions :=
      paste0('\n',
...
ensureNoDuplicateJAF_KEY <- function(dt)
  dt$JAF_KEY %>% 
  {if (any(duplicated(.)))
    stop('There are duplicates in JAF_KEY:\n',
...
Catalog <-
  openxlsx2::read_xlsx(paste0(path_to_folder_with_source_definitions,
                       ...
                       sheet=1) %>% 
  as.data.table() %>%  
  rbind(openxlsx2::read_xlsx(paste0(path_to_folder_with_source_definitions,
                                    source_of_definitions[1]),
                             sheet=2) %>% 
          as.data.table(),
        fill=TRUE) %>% 
...
CodeLines <-
  c(paste('### Compiled automatically by',Sys.getenv("USERNAME")),
    paste0('### from `',source_of_definitions[1],'`'),
    paste('###',
          c('on ' %+% Sys.time(),
            Number_of_defined_indics_message,
            Number_of_undefined_indics_message)),
    ...
    CatalogNoFormulaOrCond %>% {`if`(length(.)>0,.$definitions,"")},
    '\n')  %>% 

...
#!!!DEFINE DATASET/PATH: 
dataset <- read_csv("P:\\MicroData\\SILC\\Microdata_2024_October\\result\\SILC_C_2023_all.csv")
...
dataset <- dataset %>%
  mutate(
    new_RX010 = RX010,
    new_RX010 = if_else(COUNTRY %in% c("MT", "DE"), YEAR - new_RB080, new_RX010)
  )
...
Persons_withoutInternet_TotalPopulation_filtered <- Persons_WithoutInternet_TotalPopulation %>% 
  filter(PD080 == "No internet access")

Persons_withoutInternet_TotalPopulation_filtered$time <- 2023

Persons_withoutInternet_TotalPopulation_filtered <- Persons_withoutInternet_TotalPopulation_filtered %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
...
Persons_withoutInternet_AROPE_filtered_1 <- Persons_withoutInternet_AROPE %>%
  filter(PD080== "No internet access" & AROPE == "Yes, AROPE")

Persons_withoutInternet_AROPE_filtered_1$time <- 2023

Persons_withoutInternet_AROPE_filtered_1 <- Persons_withoutInternet_AROPE_filtered_1 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_notAROPE_filtered_2 <- Persons_withoutInternet_AROPE %>%
  filter(PD080== "No internet access" & AROPE != "Yes, AROPE")

Persons_withoutInternet_notAROPE_filtered_2$time <- 2023

Persons_withoutInternet_notAROPE_filtered_2 <- Persons_withoutInternet_notAROPE_filtered_2 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AROP_filtered_1 <- Persons_withoutInternet_AROP %>%
  filter(PD080 == "No internet access" & AROP == "Yes, AROP")

Persons_withoutInternet_AROP_filtered_1$time <- 2023

Persons_withoutInternet_AROP_filtered_1 <- Persons_withoutInternet_AROP_filtered_1 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_notAROP_filtered_2 <- Persons_withoutInternet_AROP %>%
  filter(PD080 == "No internet access" & AROP != "Yes, AROP")

Persons_withoutInternet_notAROP_filtered_2$time <- 2023

Persons_withoutInternet_notAROP_filtered_2 <- Persons_withoutInternet_notAROP_filtered_2 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AgeGroups_Ages1617 <- Persons_withoutInternet_AgeGroups %>%
  filter(PD080 == "No internet access" & age_group == "16-17 years old")

Persons_withoutInternet_AgeGroups_Ages1617$time <- 2023

Persons_withoutInternet_AgeGroups_Ages1617 <- Persons_withoutInternet_AgeGroups_Ages1617 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AgeGroups_Ages1864 <- Persons_withoutInternet_AgeGroups %>%
  filter(PD080 == "No internet access" & age_group == "18-64 years old")

Persons_withoutInternet_AgeGroups_Ages1864$time <- 2023

Persons_withoutInternet_AgeGroups_Ages1864 <- Persons_withoutInternet_AgeGroups_Ages1864 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_AgeGroups_Ages65Over <- Persons_withoutInternet_AgeGroups %>%
  filter(PD080 == "No internet access" & age_group == "65+ years")

Persons_withoutInternet_AgeGroups_Ages65Over$time <- 2023

Persons_withoutInternet_AgeGroups_Ages65Over <- Persons_withoutInternet_AgeGroups_Ages65Over %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_GENDER_filtered_Men <- Persons_withoutInternet_GENDER %>%
  filter(PD080 == "No internet access" & PB150 == "Male")


Persons_withoutInternet_GENDER_filtered_Men$time <- 2023

Persons_withoutInternet_GENDER_filtered_Men <- Persons_withoutInternet_GENDER_filtered_Men %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_GENDER_filtered_Female <- Persons_withoutInternet_GENDER %>%
  filter(PD080 == "No internet access" & PB150 == "Female")

Persons_withoutInternet_GENDER_filtered_Female$time <- 2023

Persons_withoutInternet_GENDER_filtered_Female <- Persons_withoutInternet_GENDER_filtered_Female %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_URBANISATION_Urban <- Persons_withoutInternet_URBANISATION %>%
  filter(PD080 == "No internet access" & Urbanisation == "Urban Areas")

Persons_withoutInternet_URBANISATION_Urban$time <- 2023

Persons_withoutInternet_URBANISATION_Urban <- Persons_withoutInternet_URBANISATION_Urban %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_URBANISATION_Rural <- Persons_withoutInternet_URBANISATION %>%
  filter(PD080 == "No internet access" & Urbanisation == "Rural Areas")

Persons_withoutInternet_URBANISATION_Rural$time <- 2023

Persons_withoutInternet_URBANISATION_Rural <- Persons_withoutInternet_URBANISATION_Rural %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_Education_Low <- Persons_withoutInternet_Education %>%
  filter(PD080 == "No internet access" & educ == "lower_secondary/ Low Education")


Persons_withoutInternet_Education_Low$time <- 2023

Persons_withoutInternet_Education_Low <- Persons_withoutInternet_Education_Low %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_Education_Medium <- Persons_withoutInternet_Education %>%
  filter(PD080 == "No internet access" & educ == "upper_secondary / Medium Education")

Persons_withoutInternet_Education_Medium$time <- 2023

Persons_withoutInternet_Education_Medium <- Persons_withoutInternet_Education_Medium %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutInternet_Education_High <- Persons_withoutInternet_Education %>%
  filter(PD080 == "No internet access" & educ == "tertiary/ High Education")

Persons_withoutInternet_Education_High$time <- 2023

Persons_withoutInternet_Education_High <- Persons_withoutInternet_Education_High %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_WithoutComputer_TotalPopulation_filtered <- Persons_WithoutComputer_TotalPopulation %>%
  filter(HS090 == "No computer")


Persons_WithoutComputer_TotalPopulation_filtered$time <- 2023

Persons_WithoutComputer_TotalPopulation_filtered <- Persons_WithoutComputer_TotalPopulation_filtered %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_AROP_filtered_1 <-  Persons_withoutComputer_AROP %>%
  filter(HS090 == "No computer" & AROP == "Yes, AROP")


Persons_withoutComputer_AROP_filtered_1$time <- 2023

Persons_withoutComputer_AROP_filtered_1 <- Persons_withoutComputer_AROP_filtered_1 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_notAROP_filtered_2 <-  Persons_withoutComputer_AROP %>%
  filter(HS090 == "No computer" & AROP == "No, not AROP")

Persons_withoutComputer_notAROP_filtered_2$time <- 2023

Persons_withoutComputer_notAROP_filtered_2 <- Persons_withoutComputer_notAROP_filtered_2 %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_URBANISATION_Urban <- Persons_withoutComputer_URBANISATION %>%
  filter(HS090 == "No computer" & Urbanisation == "Urban Areas")

Persons_withoutComputer_URBANISATION_Urban$time <- 2023

Persons_withoutComputer_URBANISATION_Urban <- Persons_withoutComputer_URBANISATION_Urban %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
Persons_withoutComputer_URBANISATION_Rural <- Persons_withoutComputer_URBANISATION %>%
  filter(HS090 == "No computer" & Urbanisation == "Rural Areas")

Persons_withoutComputer_URBANISATION_Rural$time <- 2023

Persons_withoutComputer_URBANISATION_Rural <- Persons_withoutComputer_URBANISATION_Rural %>%
  rename(
    geo = COUNTRY,
    value = weighted_prop
  ) %>% dplyr::select(geo, time, value)
...
#Persons_withoutInternet_TotalPopulation_filtered
#Persons_withoutInternet_AROPE_filtered_1
#Persons_withoutInternet_notAROPE_filtered_2
#Persons_withoutInternet_AROP_filtered_1
#Persons_withoutInternet_notAROP_filtered_2
#Persons_withoutInternet_AgeGroups_Ages1617
#Persons_withoutInternet_AgeGroups_Ages1864
#Persons_withoutInternet_AgeGroups_Ages65Over
#Persons_withoutInternet_GENDER_filtered_Men
#Persons_withoutInternet_GENDER_filtered_Female
#Persons_withoutInternet_URBANISATION_Urban
#Persons_withoutInternet_URBANISATION_Rural
#Persons_withoutInternet_Education_Low
#Persons_withoutInternet_Education_Medium
#Persons_withoutInternet_Education_High
#Persons_WithoutComputer_TotalPopulation_filtered
#Persons_withoutComputer_AROP_filtered_1
#Persons_withoutComputer_notAROP_filtered_2
#Persons_withoutComputer_URBANISATION_Urban
#Persons_withoutComputer_URBANISATION_Rural
